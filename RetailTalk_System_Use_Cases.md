...
| **Brief Description** | This use case allows an authenticated buyer to purchase products either through cart checkout (batch) or direct buy (single product). The system validates stock availability and wallet balance, processes payment through the internal wallet system, distributes revenue (90% seller, 10% admin commission), updates inventory, and creates transaction records. The seller receives the order for processing, and delivery personnel are assigned for delivery orders. |
...
| **Benefiting Actor(s)** | Buyer: Acquires the desired products. Seller: Receives 90% of the sale revenue and the order for processing. Delivery: Gets assigned delivery orders for fulfillment. Admin: Receives 10% commission. System (RetailTalk): Records a complete transaction for tracking and reporting. |
...
| **Post-condition(s)** | Buyer's wallet balance is deducted by the total amount. Seller receives 90% of the product price and the order appears in their transaction list. Admin receives 10% commission. Product stock is decremented. Transaction records are created with a "pending" status. Delivery personnel are notified of new delivery orders. |
...
Secondary Actors: Seller, Delivery, Admin
    ...
    <<INCLUDE>> ── [Process Wallet Payment]
         |
    <<INCLUDE>> ── [Distribute Revenue (Seller 90%, Admin 10%)]
         |
    ...
    <<INCLUDE>> ── [Notify Seller of New Order]
         |
    <<INCLUDE>> ── [Add to Delivery Queue (if delivery order)]
         |
...
| 9 | **Application:** Distribute revenue: credit 90% to each seller's balance, credit 10% to the admin's balance. |
| 10 | **Application:** Decrement product stock for each purchased item. |
| 11 | **Application:** Create transaction records for each seller group with status "pending." |
| 12 | **Application:** Notify the seller of the new incoming order. |
| 13 | **Application:** (If delivery order) Add the order to the delivery personnel queue. |
| 14 | **Application:** Clear the purchased items from the cart (if checkout). |
| 15 | **Application:** Return a purchase confirmation with transaction details. |
| 16 | **Actor (Buyer):** View the purchase confirmation and transaction details. |
...
| Step | Description |
|------|-------------|
| 1 | **Actor (Seller):** Navigate to the orders/transactions page. |
...
| 4 | **Actor (Seller):** View the list of incoming orders. |
| 5 | **Actor (Seller):** Select an order to view its full details. |
| 6 | **Application:** Display the complete order information including buyer details and payment breakdown. |
| 7 | **Actor (Seller):** (Walk-In Order) Click to update the status from "pending_walkin" to "inwork." |
| 8 | **Application:** Update the order status to "inwork" and confirm the change. |
| 9 | **Actor (Seller):** (Walk-In Order) Complete preparation and click to update the status to "ready." |
| 10 | **Application:** Update the order status to "ready" and notify the buyer. |
| 11 | **Actor (Seller):** View the updated order status and sales earnings. |
...
| 15 | **Actor (Seller):** Navigate to the wishlist analytics on the seller dashboard. |
| 16 | **Application:** Retrieve and display total wishlist count and per-product wishlist breakdown for the seller's products. |
| 17 | **Actor (Seller):** View wishlist demand insights for their products. |
...
| 3 | **Actor (Seller):** Identify a product with low stock and click "Request Restock." |
| 4 | **Actor (Seller):** Enter the requested quantity and optional notes, then submit. |
