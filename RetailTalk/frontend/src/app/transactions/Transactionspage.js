...
/**
 * Transactionspage.js — Buyer Dashboard: Spending & Orders
 */
    ...
    const loadData = async () => {
        try {
            const [txns, bal, svf] = await Promise.all([
                getTransactionHistory(), getBalance(), getSVFHistory(),
            ]);
                    ...
                    <div className="card">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                            <div style={{ textAlign: 'center', padding: 12, borderRadius: 8, background: 'rgba(16,185,129,0.08)' }}>
                                ...
                                <p style={{ fontWeight: 700, color: '#10b981' }}>₱{totalDeposits.toFixed(2)}</p>
                            </div>
                            <div style={{ textAlign: 'center', padding: 12, borderRadius: 8, background: 'rgba(239,68,68,0.08)' }}>
                                ...
                                <p style={{ fontWeight: 700, color: '#ef4444' }}>₱{totalDebits.toFixed(2)}</p>
                            </div>
                            <div style={{ textAlign: 'center', padding: 12, borderRadius: 8, background: 'rgba(99,102,241,0.08)' }}>
                                ...
                                <p style={{ fontWeight: 700, color: '#6366f1' }}>₱{(totalDeposits - totalDebits).toFixed(2)}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div style={{ display: 'grid', gap: 12, marginBottom: 20 }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                <span style={{
                                    ...
                                    background: ({
                                        ondeliver: 'rgba(59,130,246,0.15)', delivered: 'rgba(16,185,129,0.15)',
                                        undelivered: 'rgba(239,68,68,0.15)', cancelled: 'rgba(148,163,184,0.15)',
                                    })[selectedOrder.status] || 'rgba(148,163,184,0.15)',
                                    color: ({
                                        ondeliver: '#3b82f6', delivered: '#10b981', undelivered: '#ef4444',
                                    })[selectedOrder.status] || '#94a3b8',
