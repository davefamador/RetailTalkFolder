$ErrorActionPreference = "Stop"
$PYTHON = "$PSScriptRoot\..\venv\Scripts\python.exe"

$RANDOM_STATE = 42
$BATCH_SIZE = 128
$BERT_MODEL_NAME = "bert-base-multilingual-uncased"
$BERT_MAX_LENGTH = 256
$BERT_SIZE = 768
$LABELS_TYPE = "esci_labels"

$SQD_PATH = "$PSScriptRoot\..\shopping_queries_dataset\"
$DATA_REPRESENTATIONS_PATH = "$PSScriptRoot\text_representations\task2"
$DICT_PRODUCTS_PATH_FILE = "$DATA_REPRESENTATIONS_PATH\dict_products_train.npy"
$DICT_QUERIES_PATH_FILE = "$DATA_REPRESENTATIONS_PATH\dict_examples_train.npy"

$ARRAY_PRODUCTS_PATH_FILE = "$DATA_REPRESENTATIONS_PATH\array_products_train.npy"
$ARRAY_QUERIES_PATH_FILE = "$DATA_REPRESENTATIONS_PATH\array_queries_train.npy"
$ARRAY_LABELS_PATH_FILE = "$DATA_REPRESENTATIONS_PATH\array_labels_train.npy"

# 1. Get BERT representations for queries and products
if (!(Test-Path -Path $DATA_REPRESENTATIONS_PATH)) {
    New-Item -ItemType Directory -Force -Path $DATA_REPRESENTATIONS_PATH
}

Write-Host "1. Calculating BERT representations..."
& $PYTHON "$PSScriptRoot\compute_bert_representations.py" `
    $SQD_PATH `
    "train" `
    --output_queries_path_file $DICT_QUERIES_PATH_FILE `
    --output_product_catalogue_path_file $DICT_PRODUCTS_PATH_FILE `
    --model_name $BERT_MODEL_NAME `
    --bert_max_length $BERT_MAX_LENGTH `
    --batch_size $BATCH_SIZE
if ($LASTEXITCODE -ne 0) { Write-Error "Step 1 failed."; exit 1 }

Write-Host "2. Building input data model..."
& $PYTHON "$PSScriptRoot\build_input_data_model.py" `
    $SQD_PATH `
    "train" `
    $DICT_PRODUCTS_PATH_FILE `
    $DICT_QUERIES_PATH_FILE `
    $ARRAY_QUERIES_PATH_FILE `
    $ARRAY_PRODUCTS_PATH_FILE `
    $ARRAY_LABELS_PATH_FILE `
    --labels_type $LABELS_TYPE `
    --bert_size $BERT_SIZE
if ($LASTEXITCODE -ne 0) { Write-Error "Step 2 failed."; exit 1 }

$MODELS_PATH = "$PSScriptRoot\models"
$MODEL_SAVE_PATH = "$MODELS_PATH\task_2_esci_classifier_model"

# --- Improved Hyperparameters ---
$RANDOM_STATE = 42
$BATCH_SIZE = 256
$WEIGHT_DECAY = 0.01
$NUM_TRAIN_EPOCHS = 12
$LR = 1e-3
$EPS = 1e-5
$NUM_WARMUP_STEPS = -1          # Auto: 10% of total training steps
$MAX_GRAD_NORM = 1
$VALIDATION_STEPS = 100
$NUM_DEV_EXAMPLES = 5505
$DENSE_HIDDEN_DIM = 256
$NUM_DENSE_LAYERS = 2

# 3. Perform the training
Write-Host "3. Training the model..."
& $PYTHON "$PSScriptRoot\train.py" `
    $ARRAY_QUERIES_PATH_FILE `
    $ARRAY_PRODUCTS_PATH_FILE `
    $ARRAY_LABELS_PATH_FILE `
    $MODEL_SAVE_PATH `
    $LABELS_TYPE `
    --random_state $RANDOM_STATE `
    --batch_size $BATCH_SIZE `
    --weight_decay $WEIGHT_DECAY `
    --num_train_epochs $NUM_TRAIN_EPOCHS `
    --lr $LR `
    --eps $EPS `
    --num_warmup_steps $NUM_WARMUP_STEPS `
    --max_grad_norm $MAX_GRAD_NORM `
    --validation_steps $VALIDATION_STEPS `
    --num_dev_examples $NUM_DEV_EXAMPLES `
    --dense_hidden_dim $DENSE_HIDDEN_DIM `
    --num_dense_layers $NUM_DENSE_LAYERS `
    --use_class_weights
if ($LASTEXITCODE -ne 0) { Write-Error "Step 3 failed."; exit 1 }

Write-Host "Training complete! Model saved to $MODEL_SAVE_PATH"
